...
- **Schema relationships**: `User` → `Application` (`user_id` FK) → `Interview`, `Contact`, `StatusEvent` (each with an `application_id` FK). All PKs are `uuid.UUID` generated client-side via `uuid.uuid4`.
